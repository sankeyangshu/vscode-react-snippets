...
import ReactNativeSnippets, { ReactNativeSnippetType } from '../sourceSnippets/reactNative';
import ZustandSnippets, { ZustandSnippetType } from '../sourceSnippets/zustand';
import { formatSnippetPlaceholders, parseSnippetToBody } from './formatters';

/**
 * 代码片段 key 类型
 */
export type SnippetKeysType =
  | ReactComponentSnippetType['key']
  | HooksSnippetType['key']
  | ZustandSnippetType['key']
  | ReactNativeSnippetType['key'];

/**
 * 所有代码片段类型
 */
export type snippetAllType =
  | ReactComponentSnippetType
  | HooksSnippetType
  | ZustandSnippetType
  | ReactNativeSnippetType;
...
export const generateSnippets = async () => {
  try {
    ...
    const snippetList = getSnippets(scopeList.scope, scopeList.list);
